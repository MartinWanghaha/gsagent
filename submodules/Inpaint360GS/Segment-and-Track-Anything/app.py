...
import torch
import time, math
from seg_track_anything import (
    aot_model2ckpt,
    draw_mask,
    image_sequence_workspace,
    tracker_assets_root,
    tracker_image_sequence,
    tracking_objects_in_video,
    tracking_result_workspace,
)
import gc
import numpy as np
import json
...
def _sorted_files(directory, suffixes=None):
    """List regular files only after verifying that their directory exists."""
...
def _safe_extract_zip(archive_path, destination):
    """Extract a zip archive while rejecting traversal paths and symlinks."""
    ...
    with zipfile.ZipFile(archive_path) as archive:
        ...
        for member in members:
            ...
            output_path = (destination / member_path).resolve()
            if (
                member_path.is_absolute()
                or (output_path != destination and destination not in output_path.parents)
...
def get_meta_from_img_seq(input_img_seq):
    ...
    archive_path = Path(getattr(input_img_seq, "name", input_img_seq))
    file_path = image_sequence_workspace(input_img_seq)

    # Unzip file
    print(f"Extracting to: {file_path}")
    try:
        _safe_extract_zip(archive_path, file_path)
    except (OSError, ValueError, zipfile.BadZipFile) as exc:
        return None, None, None, f"Error: Failed to extract {archive_path}: {exc}"

    imgs_path = _sorted_files(file_path, _IMAGE_SUFFIXES)

    if not imgs_path:
        return None, None, None, f"Error: No images found in {file_path}"

    first_frame_path = imgs_path[0]
    first_frame = cv2.imread(str(first_frame_path))
    if first_frame is None:
        return None, None, None, f"Error: Failed to read image {first_frame_path}"
...
def res_by_num(input_video, input_img_seq, frame_num):
    if input_video is not None:
        ...
        imgs_path = _sorted_files(file_path, _IMAGE_SUFFIXES)
        if frame_num < 0 or frame_num >= len(imgs_path):
            return None, None, None
        ori_frame = imgs_path[frame_num]
        ori_frame = cv2.imread(str(ori_frame))
        ori_frame = cv2.cvtColor(ori_frame, cv2.COLOR_BGR2RGB)
    else:
        return None, None, None

    tracking_result_dir = tracking_result_workspace(video_name)
    output_masked_frame_dir = tracking_result_dir / f'{video_name}_masked_frames'
    output_masked_frame_path = _sorted_files(output_masked_frame_dir)

    output_mask_dir = tracking_result_dir / f'{video_name}_masks'
    output_mask_path = _sorted_files(output_mask_dir)


    if not output_masked_frame_path or not output_mask_path:
        return None, None, None
    else:
        if frame_num >= len(output_masked_frame_path):
            ...
            chosen_frame_show = cv2.imread(str(chosen_frame_show))
...
def show_res_by_slider(input_video, input_img_seq, frame_per):
    if input_video is not None:
        ...
        video_name = image_sequence_workspace(input_img_seq).name
    else:
...
def seg_track_app():
    ...
    ##########################################################
        ...
        video_example = tracker_assets_root() / "blackswan.mp4"
        if video_example.is_file():
            with gr.Tab(label='Video example'):
                gr.Examples(examples=[str(video_example)], inputs=[input_video])

        image_sequence_example = tracker_image_sequence()
        if image_sequence_example.is_file():
            with gr.Tab(label='Image-seq example'):
                gr.Examples(examples=[str(image_sequence_example)], inputs=[input_img_seq])
