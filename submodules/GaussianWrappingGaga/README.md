...
GaussianWrappingGaga is a standalone 3D Gaussian Splatting project that keeps
the complete Gaussian Wrapping geometry, regularization, adaptive density,
surface extraction and texture pipeline, and adds Gaga-compatible
view-consistent instance semantics as a native Gaussian field.

The integration is architectural rather than a post-render patch:

- every Gaussian owns a learnable 16D `obj_dc_*` semantic embedding;
- cloning, splitting, pruning, PLY serialization and checkpoints preserve it;
- dedicated RaDeGS and Ours CUDA rasterizers alpha-composite the 16D field;
- a scene-level 1x1 head maps rendered embeddings to associated Gaga labels;
- geometry-first lifting and joint RGB/geometry/semantic training are supported;
- RGB, depth, normals, alpha, features, logits, labels, metrics, textured mesh
  and semantic mesh are first-class outputs.

The original Gaussian Wrapping implementation remains available in full below
and is used unchanged when semantics are disabled.

## Installation

```bash
cd submodules/GaussianWrappingGaga
conda create -n gaussian_wrapping_gaga python=3.9 -y
conda activate gaussian_wrapping_gaga
...
`install.py` builds both original Gaussian Wrapping rasterizers and the two
independent semantic extensions:

- `diff_gaussian_rasterization_gw_semantic` (RaDeGS);
- `diff_gaussian_rasterization_gw_ours_semantic` (Ours).

## Input contract

The image dataset uses the same COLMAP layout as Gaussian Wrapping. Associated
Gaga masks are supplied in one directory and resolved by camera image stem:
...
encoding. `info.json` may contain `{"num_mask": N}`; class `0` is background
and the resulting decoder has `N + 1` classes. Resizing is always
nearest-neighbour.

## Recommended complete pipeline

For the repository's Mip-NeRF 360 layout, use the validated batch launchers:

```bash
# Two-stage or joint training.
...
# Geometry extraction plus semantic mesh export.
python ../../scripts/GaussianWrappingGaga/extract_gaussian_wrapping_gaga_mesh.py \
  --scene counter --mode two-stage --mask-method entityseg --gpu 0
```

See
[`scripts/GaussianWrappingGaga/README.md`](../../scripts/GaussianWrappingGaga/README.md)
for the output layout and joint-training invocation.

```bash
python gaussian_wrapping/scripts/train_extract_lift_gaga.py \
  -s <COLMAP_DATASET> \
  -m <GEOMETRY_OUTPUT> \
  --semantic_masks <GAGA_ASSOCIATED_MASKS> \
  --semantic_output <SEMANTIC_OUTPUT> \
  --rasterizer radegs \
  --eval
```

This command runs the following owned stages:

1. complete Gaussian Wrapping optimization with adaptive density control;
2. pivot-based mesh extraction and texture refinement;
3. frozen-geometry Gaga semantic lifting through the native CUDA compositor;
4. train/test rendering of every geometry and semantic product;
5. semantic mesh transfer and Hungarian/direct semantic evaluation.

Use `--rasterizer ours` for the median-depth variant. Geometry, rendering,
semantic mesh, or evaluation stages can be skipped independently. Pass
`--mesh` when a specific extracted mesh should receive semantic labels.

## Joint training

Semantics can instead participate from the start while all original Gaussian
Wrapping losses and density control remain active:

```bash
python gaussian_wrapping/train.py \
  -s <COLMAP_DATASET> \
  -m <OUTPUT> \
  --rasterizer radegs \
  --semantic_masks <GAGA_ASSOCIATED_MASKS> \
...
Resume geometry with the normal `--start_checkpoint` option and restore the
decoder/semantic optimizer with `--semantic_checkpoint`.

## Individual semantic stages

```bash
# Lift semantics into an already trained Gaussian Wrapping model.
python gaussian_wrapping/semantic_lift.py \
  -s <DATASET> -m <GEOMETRY_OUTPUT> \
  --semantic_masks <MASKS> \
  --semantic_output <SEMANTIC_OUTPUT> \
  --rasterizer radegs

# Render the complete output set.
python gaussian_wrapping/semantic_render.py \
  -s <DATASET> -m <SEMANTIC_OUTPUT> \
  --semantic_checkpoint <SEMANTIC_OUTPUT/semantic/semantic_chkpnt10000.pth> \
  --output <RENDER_OUTPUT> \
  --rasterizer radegs --split test

# Attach semantic IDs and colors to a reconstructed mesh.
python gaussian_wrapping/semantic_mesh.py \
  --mesh <GAUSSIAN_WRAPPING_MESH.ply> \
  --semantic_ply <SEMANTIC_OUTPUT/point_cloud/iteration_10000/point_cloud.ply> \
  --semantic_checkpoint <SEMANTIC_CHECKPOINT.pth> \
  --output <SEMANTIC_MESH.ply>

# Evaluate direct labels and the Gaga-style Hungarian correspondence.
python gaussian_wrapping/semantic_eval.py \
  --render_dir <RENDER_OUTPUT/test> \
  --semantic_masks <MASKS> \
  --semantic_checkpoint <SEMANTIC_CHECKPOINT.pth>
```
...
The semantic PLY is a full Gaussian Wrapping PLY plus `obj_dc_0` through
`obj_dc_15`. Nothing is discarded: SH color, opacity, scale, rotation, mip
filter, occupancy, normal features and exposure/appearance state continue to
use the original formats.
...
SEMANTIC_OUTPUT/
  ...
  semantic/
    semantic_chkpnt<N>.pth
    semantic_metadata.json
  renders/{train,test}/
    renders/                 # RGB
    ground_truth/
    depth/ and depth_npy/     # median-depth compatibility aliases
    expected_depth/ and expected_depth_npy/
    median_depth/ and median_depth_npy/
    expected_coord/ and median_coord/
    normal/ and normal_npy/
    alpha/ and alpha_npy/
    radii/
    renderer_aux/            # complete per-view renderer tensor bundle
    semantic_features/       # 16D .pt
    semantic_logits/         # C-class .pt
    semantic_labels/         # PNG or NPY
    semantic_color/
    manifest.json
  mesh/
    semantic_mesh.ply        # semantic_id + RGB vertex properties
    semantic_mesh.semantic.npy
    semantic_mesh.semantic_distance.npy
    semantic_mesh.semantic.json
```

The CUDA semantic pass reuses the exact projected conics, tile ranges and
front-to-back point ordering produced by the selected Gaussian Wrapping
rasterizer. It uses the same alpha clamp and early termination thresholds.
Semantic backward intentionally updates embeddings only, which makes the
geometry-first lift stable; the original RGB/depth/normal backward remains
fully active during joint training.

Implementation details and invariants are documented in
...
python -m py_compile gaussian_wrapping/semantic_*.py
```

GPU smoke tests automatically skip when no CUDA device is visible.

---

## Upstream Gaussian Wrapping documentation
