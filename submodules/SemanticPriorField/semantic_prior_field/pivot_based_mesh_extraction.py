...
from extraction.semantic import (
    load_semantic_state,
    resolve_semantic_checkpoint,
)
...
def marching_tetrahedra_with_binary_search(
    ...
    # Get pivots
    # > pivot_gaussian_idx maps every pivot to its generating Gaussian; it is
    #   exact for all structured strategies (including SDF-filtered
    #   intersecting pivots) and None for unstructured ones (scores/random).
    pivot_gaussian_idx = None
    if args.use_tetra_points:
        ...
        pivot_gaussian_idx = torch.arange(
            gaussians._xyz.shape[0], device=pivots.device
        ).repeat_interleave(pivots.shape[0] // gaussians._xyz.shape[0])
    else:
        if args.use_scores:
            ...
            pivot_results = get_intersecting_pivots_from_normals(
                ...
                return_gaussian_idx=args.use_semantics,
            )

        # Compute SDF values for pivots
        if args.use_intersecting_pivots:
            if args.use_semantics and len(pivot_results) == 4:
                pivots, pivot_scales, pivot_sdfs, pivot_gaussian_idx = pivot_results
            else:
            ...
            if args.use_semantics and len(pivot_results) == 3:
                pivots, pivot_scales, pivot_gaussian_idx = pivot_results
            else:
    ...
    # Attach Semantic Prior Field state to pivots via the per-pivot
    # generating-Gaussian index. The index is exact for all structured
    # strategies (including SDF-filtered intersecting pivots); unstructured
    # strategies (scores / random) have no correspondence and disable it.
    pivots_labels = None
    pivots_label_confidence = None
    pivots_semantics = None
    if args.use_semantics:
        semantic_checkpoint = args.semantic_checkpoint or resolve_semantic_checkpoint(
            model_path, args.iteration
        )
        n_gaussians = gaussians._xyz.shape[0]
        if semantic_checkpoint is None:
            print("[WARNING] No semantic sidecar checkpoint found. Disabling mesh semantics.")
        else:
            if pivot_gaussian_idx is None:
                structured = not (args.use_scores or args.random_pivots)
                if structured and pivots.shape[0] % n_gaussians == 0:
                    pivot_gaussian_idx = torch.arange(
                        n_gaussians, device=pivots.device
                    ).repeat_interleave(pivots.shape[0] // n_gaussians)
                else:
                    print("[WARNING] Pivot strategy has no Gaussian-pivot correspondence. Disabling mesh semantics.")
            if pivot_gaussian_idx is not None:
                semantic_state = load_semantic_state(
                    semantic_checkpoint,
                    gaussians,
                    min_label_confidence=args.semantic_min_label_confidence,
                )
                pivot_gaussian_idx = pivot_gaussian_idx.to(semantic_state["labels"].device)
                pivots_labels = semantic_state["labels"][pivot_gaussian_idx]
                pivots_label_confidence = semantic_state["confidence"][pivot_gaussian_idx]
                pivots_semantics = semantic_state["features"][pivot_gaussian_idx]
                print(
                    f"[INFO] Loaded mesh semantics from {semantic_checkpoint}: "
                    f"{semantic_state['num_classes']} classes, "
                    f"{pivots_labels.shape[0]} labelled pivots."
                )
                if args.filter_semantic_edges:
                    print(f"[INFO] Filtering semantic edges with confidence >= {args.semantic_edge_confidence}.")
    ...
    mesh, details = extract_mesh(
        ...
        valid=valid_mask,
        pivots_semantics=pivots_semantics,
        pivots_labels=pivots_labels,
        pivots_label_confidence=pivots_label_confidence,
        filter_semantic_edges=args.filter_semantic_edges,
        semantic_edge_confidence=args.semantic_edge_confidence,
        semantic_edge_length_ratio=args.semantic_edge_length_ratio,
    )
    torch.cuda.empty_cache()
    ...
    # Export vertex semantics (labels follow the un-postprocessed mesh)
    if args.use_semantics and details.get("verts_labels") is not None:
        verts_labels = details["verts_labels"].cpu().numpy()
        verts_label_confidence = details["verts_label_confidence"].cpu().numpy()
        semantic_export_path = mesh_save_path.replace(".ply", ".semantic.npz")
        semantic_payload = {
            "labels": verts_labels.astype(np.int32),
            "confidence": verts_label_confidence.astype(np.float32),
        }
        if args.export_vertex_semantics and details.get("verts_semantics") is not None:
            semantic_payload["embeddings"] = (
                details["verts_semantics"].cpu().numpy().astype(np.float32)
            )
        np.savez_compressed(semantic_export_path, **semantic_payload)
        print(f"[INFO] Vertex semantics saved to:\n{semantic_export_path}")

        from semantic.palette import gaga_palette
        palette = gaga_palette(int(max(verts_labels.max() + 1, 1)))
        label_colors = palette[np.clip(verts_labels, 0, palette.shape[0] - 1)]
        label_colors[verts_labels < 0] = 128
        semantic_mesh = trimesh.Trimesh(
            vertices=mesh.vertices,
            faces=mesh.faces,
            vertex_colors=label_colors,
            process=False,
        )
        semantic_mesh_path = mesh_save_path.replace(".ply", "_semantic.ply")
        semantic_mesh.export(semantic_mesh_path)
        print(f"[INFO] Semantic-colored mesh saved to:\n{semantic_mesh_path}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--use_semantics", action="store_true",
        help="Load the semantic sidecar and carry instance identity through Marching Tetrahedra.")
    parser.add_argument("--semantic_checkpoint", default="", type=str,
        help="Path to a semantic sidecar checkpoint. Defaults to <model_path>/semantic/semantic_chkpnt<iteration>.pth. "
             "(Empty string = auto-resolve; get_combined_args drops None-valued CLI args.)")
    parser.add_argument("--filter_semantic_edges", action="store_true",
        help="Drop faces whose MT edges confidently bridge two different instances.")
    parser.add_argument("--semantic_edge_confidence", default=0.5, type=float)
    parser.add_argument("--semantic_edge_length_ratio", default=0.5, type=float,
        help="Semantic-bad edges are dropped only if longer than this ratio of the pivot scale sum (short label-crossing edges are real contact surfaces).")
    parser.add_argument("--semantic_min_label_confidence", default=0.3, type=float)
    parser.add_argument("--export_vertex_semantics", action="store_true",
        help="Also export interpolated 16D vertex embeddings into the .semantic.npz.")
