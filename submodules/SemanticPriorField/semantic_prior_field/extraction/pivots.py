...
def get_intersecting_pivots_from_normals(
    ...
    return_gaussian_idx: bool = False,
    ...
    if sdf_function is not None:
        ...
        if return_gaussian_idx:
            # Per-pivot generating-Gaussian index, filtered and ordered
            # exactly like the pivots above (Semantic Prior Field mesh path)
            all_idx = torch.arange(xyz.shape[0], device=pivots.device)
            pivot_gaussian_idx = torch.cat(
                [
                    all_idx.unsqueeze(1).expand(-1, intersection_mask.shape[1])[intersection_mask],
                    all_idx[intersection_mask.any(dim=-1)]
                ],
                dim=0,
            )
            return pivots, pivot_scales, pivot_sdf, pivot_gaussian_idx
    ...
    if return_gaussian_idx:
        pivot_gaussian_idx = torch.arange(
            xyz.shape[0], device=pivots.device
        ).repeat_interleave(n_pivots)
        return pivots, pivot_scales, pivot_gaussian_idx
