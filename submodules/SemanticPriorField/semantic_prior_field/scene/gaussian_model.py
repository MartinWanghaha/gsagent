...
class GaussianModel:
    ...
    def __init__(
        ...
        self.use_semantic_features = semantic_dim > 0
    ...
    def capture(self):
        ...
        if self.use_semantic_features:
            to_return += (self._semantic_features,)
        return to_return
    ...
    def restore(self, model_args, training_args):
        ...
        if self.use_semantic_features and start_idx < len(model_args):
            candidate = model_args[start_idx]
            if isinstance(candidate, torch.Tensor) and candidate.ndim == 2 and candidate.shape[1] == self.semantic_dim:
                self._semantic_features = candidate
                start_idx = start_idx + 1
        ...
        if self.use_semantic_features and self._semantic_features.numel() == 0:
            self.initialize_semantic_features(self.semantic_dim)
        ...
        try:
            ...
            can_upgrade_legacy_checkpoint = (
                self.use_semantic_features
                and len(current_groups) == len(saved_groups) + 1
                and current_groups[-1].get("name") == "semantic_features"
            )
            if not can_upgrade_legacy_checkpoint:
                raise
            upgraded = copy.deepcopy(opt_dict)
            upgraded["param_groups"].append(current_groups[-1])
            self.optimizer.load_state_dict(upgraded)
            print(
                "[INFO] Upgraded legacy Gaussian optimizer checkpoint with a "
                "new semantic_features parameter group."
            )
        if self.use_appearance_network:
    ...
    def get_semantic_features(self):
        if not self.use_semantic_features:
            raise RuntimeError("Semantic features have not been initialized")
        return self._semantic_features

    def initialize_semantic_features(self, semantic_dim: int = 16, std: float = 1e-2):
        if semantic_dim != 16:
            raise ValueError("The CUDA semantic rasterizers require semantic_dim=16")
        if self._xyz.numel() == 0:
            raise RuntimeError("Initialize the Gaussian geometry before semantic features")
        self.semantic_dim = semantic_dim
        self.use_semantic_features = True
        features = torch.empty(
            (self._xyz.shape[0], semantic_dim),
            dtype=torch.float32,
            device=self._xyz.device,
        )
        torch.nn.init.normal_(features, mean=0.0, std=std)
        self._semantic_features = nn.Parameter(features.requires_grad_(True))

    def semantic_training_setup(self, learning_rate: float):
        if not self.use_semantic_features or self._semantic_features.numel() == 0:
        ...
        if self.learn_occupancy:
            self._occupancy_shift.requires_grad_(False)
        if self.use_gaussian_features:
            self._gaussian_features.requires_grad_(False)
        self._semantic_features.requires_grad_(True)
        self.optimizer = Adam(
            [{
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float):
        ...
        if self.use_semantic_features:
            semantic_features = torch.empty(
                (self._xyz.shape[0], self.semantic_dim),
                device="cuda",
            )
            torch.nn.init.normal_(semantic_features, mean=0.0, std=1e-2)
    ...
    def training_setup(self, training_args):
        ...
        if self.use_semantic_features:
            semantic_lr = getattr(training_args, "semantic_lr", training_args.feature_dc_lr)
            l.append({
                'params': [self._semantic_features],
                'lr': semantic_lr,
                "name": "semantic_features",
    ...
    def save_ply(self, path):
        ...
        if self.use_semantic_features:
            to_concatenate = to_concatenate + (semantic_features,)
    ...
    def load_ply(self, path):
        ...
        if len(semantic_feature_names) > 0:
            ...
            if len(semantic_feature_names) != 16:
                raise ValueError(
                    f"Expected 16 obj_dc semantic fields, got {len(semantic_feature_names)}"
                )
            print("[INFO] Loading Gaga-compatible semantic features from ply file.")
            self.semantic_dim = len(semantic_feature_names)
            self.use_semantic_features = True
            semantic_features = np.zeros((xyz.shape[0], self.semantic_dim))
            for idx, attr_name in enumerate(semantic_feature_names):
                semantic_features[:, idx] = np.asarray(
                    plydata.elements[0][attr_name]
                )
        elif self.use_semantic_features:
            semantic_features = np.empty((xyz.shape[0], self.semantic_dim))
            rng = np.random.default_rng(0)
            semantic_features[:] = rng.normal(
                loc=0.0,
                scale=1e-2,
                size=semantic_features.shape,
            )
            print("[INFO] Initialized semantic features for a legacy PLY.")
        else:
            print("[INFO] No semantic features found in ply file.")
    ...
    def densification_postfix(
        ...
        if self.use_semantic_features:
            if new_semantic_features is None:
                new_semantic_features = torch.zeros(
                    (new_xyz.shape[0], self.semantic_dim),
                    dtype=self._semantic_features.dtype,
                    device=self._semantic_features.device,
                )
    ...
    def densify_and_clone_from_mask(
        ...
        new_semantic_features: Optional[torch.Tensor]=None,
    ):
        if new_xyz is None:
    ...
    def densify_and_clone(self, grads, grad_threshold, scene_extent):
        ...
        new_semantic_features = (
            self._semantic_features[selected_pts_mask]
            if self.use_semantic_features else None
        )
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2):
        ...
        new_semantic_features = (
            self._semantic_features[selected_pts_mask].repeat(N, 1)
            if self.use_semantic_features else None
        )
    ...
    def reinitial_pts(self, pts, rgb):
        ...
        if self.use_semantic_features:
            semantic_features = torch.empty(
                (fused_point_cloud.shape[0], self.semantic_dim),
                dtype=torch.float,
                device="cuda",
            )
            torch.nn.init.normal_(semantic_features, mean=0.0, std=1e-2)
    ...
    def densify_and_split_radegs(
        ...
        n_init_points = self.get_xyz.shape[0]
        # Per-Gaussian threshold multipliers (Semantic Prior Field budget
        # reallocation); Gaussians appended since the stats were gathered
        # get a neutral multiplier of 1.
        if grad_multipliers is not None:
            padded_multipliers = torch.ones((n_init_points), device="cuda")
            padded_multipliers[:grad_multipliers.shape[0]] = grad_multipliers
        else:
            padded_multipliers = 1.0
        # Extract points that satisfy the gradient condition
        padded_grad = torch.zeros((n_init_points), device="cuda")
        padded_grad[:grads.shape[0]] = grads.squeeze()
        selected_pts_mask = torch.where(padded_grad >= grad_threshold * padded_multipliers, True, False)
        if use_abs_grad:
            padded_grad_abs = torch.zeros((n_init_points), device="cuda")
            padded_grad_abs[:grads_abs.shape[0]] = grads_abs.squeeze()
            selected_pts_mask_abs = torch.where(padded_grad_abs >= grad_abs_threshold * padded_multipliers, True, False)
    ...
    def densify_and_clone_radegs(
        ...
        # Per-Gaussian threshold multipliers (Semantic Prior Field budget
        # reallocation)
        if grad_multipliers is None:
            grad_multipliers = 1.0
        # Extract points that satisfy the gradient condition
        selected_pts_mask = torch.where(torch.norm(grads, dim=-1) >= grad_threshold * grad_multipliers, True, False)
        if use_abs_grad:
            selected_pts_mask_abs = torch.where(torch.norm(grads_abs, dim=-1) >= grad_abs_threshold * grad_multipliers, True, False)
            selected_pts_mask = torch.logical_or(selected_pts_mask, selected_pts_mask_abs)
        selected_pts_mask = torch.logical_and(selected_pts_mask,
                                              torch.max(self.get_scaling, dim=1).values <= self.percent_dense*scene_extent)
    ...
    def densify_and_prune_radegs(
        ...
        self.densify_and_clone_radegs(grads, max_grad, grads_abs, Q, extent, use_abs_grad=use_abs_grad, grad_multipliers=grad_multipliers)
        clone = self._xyz.shape[0]

        self.densify_and_split_radegs(grads, max_grad, grads_abs, Q, extent, use_abs_grad=use_abs_grad, grad_multipliers=grad_multipliers)
