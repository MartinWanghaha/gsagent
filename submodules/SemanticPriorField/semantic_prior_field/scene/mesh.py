    ...
    def __init__(
        ...
        if use_opengl:
            try:
                ...
                # Headless environments (no display / EGL below GL 4.4)
                # cannot create a GL context; the CUDA rasterizer produces
                # the same outputs.
                print(f"[WARNING] OpenGL context unavailable ({gl_error}); "
                      "falling back to the nvdiffrast CUDA context.")
