...
from gaussian_renderer.semantic_runtime import (
    load_semantic_rasterizer,
    load_semantic_rasterizer_with_fallback,
)
...
def render_ours(
    ...
    semantic_stats_sink : Optional[dict] = None,
):
    """
    Render the scene. 
    ...
    settings_class = GaussianRasterizationSettings
    rasterizer_class = GaussianRasterizer
    semantic_backend = None
    if render_semantics:
        if not pc.use_semantic_features:
            raise RuntimeError("render_semantics=True but the Gaussian model has no semantics")
        # The SPF backend additionally accumulates per-Gaussian conflict and
        # contribution statistics in its semantic backward; it is selected
        # whenever a stats sink is supplied.
        requested_backend = "spf" if semantic_stats_sink is not None else "ours"
        semantic_module, semantic_backend = load_semantic_rasterizer_with_fallback(
            requested_backend
        )
        settings_class = semantic_module.GaussianRasterizationSettings
        rasterizer_class = semantic_module.GaussianRasterizer
    ...
    if semantic_backend == "spf" and semantic_stats_sink is not None:
        settings_kwargs["stats_sink"] = semantic_stats_sink
    raster_settings = settings_class(**settings_kwargs)
    ...
    if render_semantics:
        result["semantic_features"] = rendered_semantic
    return result
