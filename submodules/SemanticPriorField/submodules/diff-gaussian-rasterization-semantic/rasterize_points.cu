	...
	const bool debug)
{
  ...
  if(P != 0)
  {
	  ...
	  char* image_chunk = reinterpret_cast<char*>(imgBuffer.contiguous().data_ptr());
	  ...
	  CudaRasterizer::ImageState image_state =
		  CudaRasterizer::ImageState::fromChunk(image_chunk, W * H);
  ...
  return std::make_tuple(rendered, out_color, out_semantic, out_coord, out_mcoord, out_alpha, out_normal, out_depth, out_mdepth, radii, geomBuffer, binningBuffer, imgBuffer);
}
	...
	const bool debug) 
{
  ...
  {  
	  ...
	  char* geom_chunk = reinterpret_cast<char*>(geomBuffer.contiguous().data_ptr());
	  char* binning_chunk = reinterpret_cast<char*>(binningBuffer.contiguous().data_ptr());
	  char* image_chunk = reinterpret_cast<char*>(imageBuffer.contiguous().data_ptr());
	  CudaRasterizer::GeometryState geom_state =
		  CudaRasterizer::GeometryState::fromChunk(geom_chunk, P);
	  CudaRasterizer::BinningState binning_state =
  ...
  return std::make_tuple(dL_dmeans2D, dL_dcolors, dL_dsemantic, dL_dopacity, dL_dmeans3D, dL_dcov3D, dL_dsh, dL_dscales, dL_drotations);
}
