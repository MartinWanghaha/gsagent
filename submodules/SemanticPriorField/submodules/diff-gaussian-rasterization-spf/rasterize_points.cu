...
#include "cuda_rasterizer/semantic.h"
#include <cstdio>
...
RasterizeGaussiansCUDA(
    ...
    const torch::Tensor& semantic_features,
    ...
    if (semantic_features.ndimension() != 2
        || semantic_features.size(0) != means3D.size(0)
        || semantic_features.size(1) != SEMANTIC::NUM_SEMANTIC_CHANNELS) {
        AT_ERROR("semantic_features must have dimensions (num_points, 16)");
    }
    ...
    torch::Tensor out_semantic = torch::full(
        {SEMANTIC::NUM_SEMANTIC_CHANNELS, H, W}, 0.0, float_opts);
    ...
    if (P != 0) {
        ...
        char* tile_chunk = reinterpret_cast<char*>(tileBuffer.contiguous().data_ptr());
        CudaRasterizer::GeometryState geom_state =
            CudaRasterizer::GeometryState::fromChunk(geom_chunk, P);
        CudaRasterizer::BinningState binning_state =
            CudaRasterizer::BinningState::fromChunk(binning_chunk, rendered);
        const dim3 tile_grid(
            (W + BLOCK_X - 1) / BLOCK_X,
            (H + BLOCK_Y - 1) / BLOCK_Y,
            1);
        CudaRasterizer::TileState<false> tile_state =
            CudaRasterizer::TileState<false>::fromChunk(
                tile_chunk, tile_grid.x * tile_grid.y);
        const dim3 semantic_block(BLOCK_X, BLOCK_Y, 1);
        SEMANTIC::forward(
            tile_grid,
            semantic_block,
            tile_state.gaussian_ranges,
            binning_state.point_list,
            W,
            H,
            geom_state.means2D,
            geom_state.conic_opacity,
            semantic_features.contiguous().data_ptr<float>(),
            out_semantic.contiguous().data_ptr<float>());
    }
    return std::make_tuple(rendered, out_color, out_semantic, out_alpha, out_normal, out_mdepth, radii, geomBuffer, binningBuffer, imgBuffer, tileBuffer);
}
...
RasterizeGaussiansBackwardCUDA(
    ...
    torch::Tensor dL_dsemantic     = torch::zeros(
        {P, SEMANTIC::NUM_SEMANTIC_CHANNELS}, means3D.options());
    torch::Tensor stat_semantic_abs_grad     = torch::zeros({P}, means3D.options());
    torch::Tensor stat_semantic_contribution = torch::zeros({P}, means3D.options());
    ...
    if (P != 0) {
        ...
        CudaRasterizer::GeometryState geom_state =
            CudaRasterizer::GeometryState::fromChunk(geom_chunk, P);
        CudaRasterizer::BinningState binning_state =
            CudaRasterizer::BinningState::fromChunk(binning_chunk, R);
        const dim3 tile_grid(
            (W + BLOCK_X - 1) / BLOCK_X,
            (H + BLOCK_Y - 1) / BLOCK_Y,
            1);
        ...
        SEMANTIC::backward(
            ...
            geom_state.means2D,
            geom_state.conic_opacity,
            dL_dout_semantic.contiguous().data_ptr<float>(),
            dL_dsemantic.contiguous().data_ptr<float>(),
            stat_semantic_abs_grad.contiguous().data_ptr<float>(),
            stat_semantic_contribution.contiguous().data_ptr<float>());
    }

    return std::make_tuple(dL_dmeans2D, dL_dcolors, dL_dsemantic, dL_dopacity, dL_dmeans3D, dL_dcov3D, dL_dsh, dL_dsg_axis, dL_dsg_sharpness, dL_dsg_color, dL_dscales, dL_drotations, stat_semantic_abs_grad, stat_semantic_contribution);
}
