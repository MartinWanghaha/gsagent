...
semanticBackwardCUDA(
	...
	float* __restrict__ stat_abs_grad,
	float* __restrict__ stat_contribution)
{
	auto block = cg::this_thread_block();
	const uint32_t horizontal_blocks = (width + BLOCK_X - 1) / BLOCK_X;
	const uint2 pix = {
	...
	float pixel_gradient_norm = 0.0f;
	if (inside)
	{
		for (int channel = 0; channel < CHANNELS; ++channel)
		{
			const float g = grad_out_semantic[channel * height * width + pix_id];
			pixel_gradient[channel] = g;
			pixel_gradient_norm += g * g;
		}
		pixel_gradient_norm = sqrtf(pixel_gradient_norm);
	}
	...
	for (int round = 0; round < rounds; ++round, to_do -= SEMANTIC_BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(SEMANTIC_BLOCK_SIZE, to_do); ++j)
		{
			...
			// Semantic Prior Field statistics: unsigned gradient mass and
			// visibility mass (cannot cancel across pixels, unlike the
			// signed embedding gradient above).
			atomicAdd(&stat_abs_grad[gaussian_id], weight * pixel_gradient_norm);
			atomicAdd(&stat_contribution[gaussian_id], weight);
			transmittance = next_transmittance;
		}
	}
}
