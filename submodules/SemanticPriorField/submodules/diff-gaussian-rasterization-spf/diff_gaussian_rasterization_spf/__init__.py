...
class _RasterizeGaussians(torch.autograd.Function):
    ...
    def forward(
        ...
        if raster_settings.debug:
            ...
            try:
                num_rendered, color, semantic, alpha, normal, mdepth, radii, geomBuffer, binningBuffer, imgBuffer, tileBuffer = _C.rasterize_gaussians(*args)
        ...
        return color, semantic, radii, mdepth, alpha, normal

    @staticmethod
    def backward(ctx, grad_color, grad_semantic, grad_radii, grad_mdepth, grad_alpha, grad_normal):
        ...
        if raster_settings.debug:
            ...
            try:
                (
                    ...
                    grad_scales,
                    grad_rotations,
                    stat_semantic_abs_grad,
                    stat_semantic_contribution,
        ...
        # Semantic Prior Field statistics: delivered through the optional
        # stats_sink so density control can consume them without touching
        # the autograd contract.
        sink = getattr(raster_settings, "stats_sink", None)
        if sink is not None:
            sink["semantic_abs_grad"] = stat_semantic_abs_grad
            sink["semantic_contribution"] = stat_semantic_contribution
            sink["semantic_grad"] = grad_semantic_features
