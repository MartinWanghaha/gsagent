...
def _scaled_dot_product_attention(query, key, value, dropout_p=0.0, scale=None):
    """Call fused SDPA with explicit-scale semantics on PyTorch 2.0+.

    PyTorch 2.0 provides fused SDPA but does not accept its later ``scale``
    keyword.  Pre-scaling the query by ``scale / default_scale`` is
    mathematically equivalent and works on both old and new APIs.  UniCeption's
    normal attention scale equals the default, so that common path incurs no
    extra tensor operation.
    """

    default_scale = query.shape[-1] ** -0.5
    if scale is not None and scale != default_scale:
